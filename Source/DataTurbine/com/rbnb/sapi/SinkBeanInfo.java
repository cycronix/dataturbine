...
/**
  * Provides information about the Sink class for JavaBeans
...
/* public */ class SinkBeanInfo extends SimpleBeanInfo
{
	MethodDescriptor[] md;
	...
	public SinkBeanInfo()
	{
		...
		Method[] methods=Sink.class.getMethods();
...
//				&&m.getParameterTypes().length==0
			...
			if (m.getName().equals("OpenRBNBConnection")
				...
				continue;
			if (m.getName().equals("Request")
				&&m.getParameterTypes().length!=4)
				continue;
			if (m.getName().equals("RequestRegistration")
				&&m.getParameterTypes().length!=1)
				continue;
