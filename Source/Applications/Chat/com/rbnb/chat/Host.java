...
/**
 * Chat room host class.
 * <p>
 * This class provides the capability to host a chat room. A host has two
...

    /**
     * Adds the specified sender to the user mappings for the room.
     * <p>
     *
     * @author Ian Brown
     *
     * @param roomI	    the chat room.
     ...
     *
     */
    private final void addSender(String roomI,
				 String senderI,
				 String fullSenderI,
				 long messageCountI)
    {
	java.util.Hashtable table =
	    (java.util.Hashtable) userMapping.get(roomI);
	FullUser fullUser = new FullUser(fullSenderI,messageCountI);

	if (table == null) {
	    table = new java.util.Hashtable();
	    userMapping.put(roomI,table);
	}
...

     ...
     *
     */
    private final long checkSender(String roomI,
				   String senderI,
				   String fullSenderI)
    {
	java.util.Hashtable table =
	...
	if (table == null) {
	    addSender(roomI,senderI,fullSenderI,messageCount);
	} else {
...

     ...
     * MM/DD/YYYY
     * ----------  --	-----------
     * 08/08/2005  EMF  Added MIME type to help plot and other clients
     * 04/09/2003  INB	Allow chat users to specify their name by adding an
     *			extra level to the channel name.
	...
	for (idx = 0; idx < messageI.NumberOfChannels(); ++idx) {
	    chatRoom = messageI.GetName(idx);
	    slash = chatRoom.indexOf("/");

	    if (slash == -1) {
		rSender = sender;
		rFullSender = fullSender;
	    } else {
		rSender = chatRoom.substring(slash + 1);
		rFullSender = null;
		chatRoom = chatRoom.substring(0,slash);
	    }

	    if ((messageCount = checkSender(chatRoom,
					    rSender,
					    rFullSender)) == 0) {
	    ...
	    } else if ((messageI.GetType(idx) !=
			com.rbnb.sapi.ChannelMap.TYPE_STRING) &&
		       (messageI.GetType(idx) !=
			com.rbnb.sapi.ChannelMap.TYPE_BYTEARRAY)) {
		messageI.PutDataAsString
		    (idx,
		     (getFancyReceipts() ?
		      "Rejected bad data type for " +
		      rSender + 
		      ", type is " +
		      messageI.TypeName(messageI.GetType(idx)) + 
		      ", reference is "+messageI.GetRequestReference() :
		      "Rejected."));
		...
		if (messageI.GetType(idx) ==
		    ...
		    byte[][] data = messageI.GetDataAsByteArray(idx);
		    message = new String[data.length];
		    for (int idx1 = 0; idx1 < message.length; ++idx1) {
			message[idx1] = new String(data[idx1]);
		    }
		}

		cmap.Clear();
		cmap.Add(chatRoom + "/" + rSender);
		cmap.PutTimeAuto("timeofday");
		for (int idx1 = 0; idx1 < message.length; ++idx1) {
		    cmap.PutDataAsString(0,message[idx1]);
                    //EMF 8/8/05: set MIME type
                    cmap.PutMime(0,"text/plain");
		    if (message[idx1].equals
			(rSender + " has left this group.")) {
			removeSender(chatRoom,rSender,rFullSender);
		    }
		}
		if (source != null) {
...

    /**
     * Removes the specified sender from the user mappings for the room.
     * <p>
     *
     * @author Ian Brown
     *
     ...
     * @param senderI	  the name of the sender.
     * @param fullSenderI the full name of the sender.
     * @see #addSender(String roomI,String senderI,String fullSenderI)
     * @since V2.0
     * @version 01/16/2003
     */
     ...
     *
     */
    private final void removeSender(String roomI,
				    String senderI,
				    String fullSenderI)
    {
	java.util.Hashtable table =
