...
%  src = rbnb_source(host, 'mySource', ncache, 'create', narchive*2);
  src = com.rbnb.sapi.Source(ncache, 'create', narchive*2);
  src.OpenRBNBConnection(host, 'mySource', username, password);
...
%  snk = rbnb_sink(host,'mySink');
  ...
  snk.OpenRBNBConnection(host, 'mySink', username, password);
...
% try detaching and re-loading archive
  src.Detach;
...
%  src = rbnb_source(host, 'mySource', ncache, 'append', narchive + narchive2);
  src = com.rbnb.sapi.Source(ncache, 'append', narchive + narchive*2);
%  src.OpenRBNBConnection(host, 'mySource', username, password);
 src.OpenRBNBConnection(host, 'mySource');
