...
public abstract class KmlMonitor extends HttpMonitor
{
	...
	protected static class KmlParser extends RootParser
	{
		...
		/**
		  * Utility to create a path based on stored folders.
		  * @since 2006/09/21
		  */
		private static String generateName(Stack folders, String name)
		{
			StringBuffer gen = new StringBuffer();
			
			for (java.util.Iterator iter = folders.iterator(); iter.hasNext(); ) {
			...
			try {
				...
				if ("name".equals(qName) && !inPlacemark) {
					try {
						...
						name = newName; 
						/* 2006/09/21  WHF  Obsolete by folder system:						
					...
					*/
					String qualname = generateName(folderStack, name);
					if (netLink) {
						try {
